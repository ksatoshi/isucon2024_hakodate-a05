...
def app_post_users(
    ...
    with engine.begin() as conn:
        ...
        if req.invitation_code:
            ...
            check = conn.execute(
                text("SELECT CASE WHEN COUNT(*) <= 2 AND EXISTS (SELECT * FROM users WHERE invitation_code = :invitation_code) THEN TRUE ELSE FALSE END AS result FROM coupons WHERE code = :code FOR UPDATE"),
                {"code": "INV_" + req.invitation_code, "invitation_code": req.invitation_code},
            ).fetchall()
...
def app_post_rides(
    ...
    with engine.begin() as conn:
        ...
        if ride_count == 1:
            ...
            coupon = conn.execute(
                text(
                    "SELECT * FROM coupons WHERE user_id = :user_id AND used_by IS NULL ORDER BY CASE code WHEN 'CP_NEW2024' THEN 1 ELSE 2 END, created_at LIMIT 1 FOR UPDATE"
                ),
                {"user_id": user.id},
            ).fetchone()
            ...
            # former code is here (comment outed -- ke-)
            '''
            coupon = conn.execute(
                text(
...
def get_chair_stats(
    ...
    query = conn.execute(
        text(
            """
            SELECT
                COUNT(evaluation) AS ride_count,
                AVG(evaluation) AS average_evaluation
            FROM
                rides
            WHERE
                chair_id = :chair_id
                AND EXISTS ( SELECT * FROM ride_statuses WHERE rides.id = ride_statuses.ride_id AND ride_statuses.status = "ARRIVED" )
                AND EXISTS ( SELECT * FROM ride_statuses WHERE rides.id = ride_statuses.ride_id AND ride_statuses.status = "COMPLETED" )
            """
        ),
        {"chair_id": chair_id},
    ).fetchone()
    ...
    return AppGetNotificationResponseChairStats(
        total_rides_count=query[0], total_evaluation_avg=query[1]
    )
    # change ends here
    ...
    for ride in rides:
        ...
        for status in ride_statuses:
            if status.status == "ARRIVED":
                ...
                is_completed = True
            if is_arrived and is_completed:
                is_countable = True
                break
